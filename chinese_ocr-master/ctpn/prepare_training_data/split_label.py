...
# out_path = 're_image'
def split_labels(path, gt_path, out_path):
    ...
    for file in files:
        ...
        print('[{}/{}]{}'.format(count, all_file_count, img_path))
        ...
        for line in lines:
            ...
            # idx = list(filter(lambda i : x_left[i] == x_right[i], range(len(x_left))))
            idx = list(filter(lambda i : x_left[i] == x_right[i], range(min(len(x_left), len(x_right)))))
